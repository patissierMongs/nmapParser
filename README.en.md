...
2. **19-column CSV.** host/OS/proto plus 식별·**위험도(상/중/하)**·분류·용도·노출위험·공격표면·**출처(KISA/CIS/MITRE)** — filter `위험도=상` in Excel for instant prioritization.
...
| **OS** | nmap `-O` best osmatch (e.g. `Linux 5.X (95%)`) |
| **PORT** / **프로토콜** / **포트상태** | nmap basics |
...
| **용도** | `관리` / `사용자` / `시스템` / `모니터링` / ... |
| **위험도** | `상` / `중` / `하` (KISA-style enum) |
| **노출위험** | one-line risk if exposed externally (e.g. "plaintext credentials") |
| **공격표면** | known attack techniques / CVEs (e.g. "EternalBlue (MS17-010), NTLM relay") |
| **출처** | reference standards — `KISA U-21, CIS 4.5, MITRE T1040` etc. |
...
| `categories.xlsx` | service / category / usage / risk / exposure / surface / source / desc (8 col) | auto-fills 분류·용도·위험도·노출위험·공격표면·출처 (105 entries with KISA+CIS+MITRE). Older 3/4/6-col files auto-detected and back-filled from code dict. To force 8-col upgrade run `python scripts/migrate_categories_to_8col.py` |
