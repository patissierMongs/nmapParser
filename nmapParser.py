...
PRESETS_JSON_NAME = "presets.json"
...
class NmapParserApp:
    def __init__(self, root):
        ...
        self.presets_json_path = os.path.join(here, PRESETS_JSON_NAME)
    ...
    def _build_static_ui(self):
        ...
        tk.Label(csv_frame, text=
                 "  요약CSV(_summary): 포트상태/서비스/식별/분류/용도/비고 + 대상그룹/조치상태",
                 fg="#555").pack(side="left", padx=4)

        # 7. 프리셋
        preset_row = tk.LabelFrame(self.root, text="프리셋")
        tk.Label(preset_row, text="선택:").pack(side="left", padx=4)
        self.preset_var = tk.StringVar(value="")
        self.preset_combo = ttk.Combobox(preset_row, textvariable=self.preset_var, width=24, state="readonly")
        self.preset_combo.pack(side="left", padx=4)
        tk.Button(preset_row, text="적용", command=self._apply_selected_preset, padx=8).pack(side="left", padx=2)
        tk.Button(preset_row, text="현재값 저장", command=self._save_current_as_preset, padx=8).pack(side="left", padx=2)
        tk.Button(preset_row, text="삭제", command=self._delete_selected_preset, padx=8).pack(side="left", padx=2)
        ...
        tk.Button(run, text="Phase1 기준 적용", command=self._apply_phase1_profile, padx=10, pady=4).pack(side="left", padx=2)
    ...
    def _is_valid_hostname(self, host):
        if not host or len(host) > 253:
            return False
        if re.match(r"^\d+\.\d+\.\d+\.\d+$", host.strip()):
            return False
    ...
    def _default_presets(self):
        return {
            "phase1": {
                "options": [
                    "-Pn", "-n", "-sS", "-sU", "-sV", "--version-all",
                    "UDP 주요 포트 스캔", "TCP 모든 포트 (1-65535)",
                    "--min-hostgroup 64", "--max-parallelism 100",
                    "-T4", "--max-retries 2", "--reason", "--open", "--defeat-rst-ratelimit"
                ],
                "custom_ports": "T:1-65535,U:7,53,67,68,69,88,123,135,137,138,139,161,162,389,400,500,514,520,623,1900,2049,4500,5060,5353,5355,11211",
                "custom_nse": "http-headers,http-server-header,http-title,ssh-hostkey,ssl-cert,ssl-enum-ciphers,tls-alpn,ms-sql-info,oracle-tns-version,rdp-ntlm-info,snmp-info,ike-version,sip-methods,ntp-info,nbstat,smb-os-discovery,smb-protocols,rpcinfo,fingerprint-strings"
            }
        }

    def _load_presets(self):
        self.presets = self._default_presets()
        if not os.path.isfile(self.presets_json_path):
            return
        try:
    ...
    def _save_presets(self):
        data = {k: v for k, v in self.presets.items() if k != "phase1"}
        try:
            with open(self.presets_json_path, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except OSError as e:
            messagebox.showwarning("프리셋 저장 실패", str(e))
    ...
    def _apply_selected_preset(self):
        ...
        if not preset:
            return
        wanted = set(preset.get("options", []))
        for entry in self.option_vars:
            row = entry["row"]
            opt = row["option"]
            label = row["label"]
            if entry["kind"] == "checkbox":
                entry["var"].set(opt in wanted or label in wanted)
            elif entry["kind"] == "radio":
                if opt in wanted or label in wanted:
                    self.group_vars[entry["group"]].set(opt)
        self.custom_ports.set(preset.get("custom_ports", ""))
        self.custom_nse.set(preset.get("custom_nse", ""))

    def _save_current_as_preset(self):
        name = simpledialog.askstring("프리셋 저장", "프리셋 이름:")
        if not name:
            return
        options = [r["option"] for r in self._gather_active_rows()]
        self.presets[name] = {
            "options": options,
            "custom_ports": self.custom_ports.get().strip(),
    ...
    def _delete_selected_preset(self):
        name = (self.preset_var.get() or "").strip()
        if not name or name == "phase1":
            messagebox.showinfo("삭제 불가", "phase1 기본 프리셋은 삭제할 수 없습니다.")
            return
    ...
    def _apply_phase1_profile(self):
        """phase1 기본 프리셋 빠른 적용 버튼."""
        self.preset_var.set("phase1")
        self._apply_selected_preset()
        messagebox.showinfo("Phase1 적용",
            "요청하신 phase1_tcp_identification 기준 옵션을 적용했습니다.\n"
            "필요 시 타겟만 입력 후 바로 실행하세요.")
    ...
    def validate_option_conflicts(self, extra_tokens):
        ...
        p_count = sum(1 for t in extra_tokens if t == "-p" or t.startswith("-p"))
        if p_count >= 2:
            result.issues.append(ValidationIssue(
                code="OPTION_CONFLICT_PORTSPEC",
                field="options",
                message="여러 -p 지정이 동시에 선택되었습니다. (nmap은 마지막 -p만 적용)",
                hint="포트 옵션은 하나만 선택하거나 custom 포트 칸으로 통합 입력하세요."
            ))
        return result
    ...
    def _scan_done(self, rc):
        ...
        if rc == 0 and self.csv_convert.get():
            xml_path = (self.output_prefix or "") + ".xml"
            if os.path.isfile(xml_path):
                try:
                    csv_paths = self._convert_to_csv(xml_path)
                    if csv_paths:
                        files_made.extend(csv_paths)
        ...
        elif rc != 0 and self.csv_convert.get():
            self._append_log("\n[안내] 비정상 종료/사용자 중지로 CSV 변환을 건너뜁니다.\n")
    ...
    def _convert_to_csv(self, xml_path):
            ...
            for port in ports_el.findall("port"):
                ...
                if svc_el is not None:
                    ...
                    if method == "probed":
                        ...
                        probed = name
                    confirmed_simple = name.strip()
                else:
                    probed = ""
                    confirmed_simple = ""
                service_group, service_purpose, service_note = self._service_profile_from_name(confirmed_simple or guessed)
                scripts = port.findall("script")
                script_names = []
                if not scripts:
                    rows.append([addr, portid, state, guessed, confirmed_simple, service_group, service_purpose, service_note, probed, "", ""])
                else:
                    for sc in scripts:
                        sid = sc.get("id", "") or ""
                        if sid:
                            script_names.append(sid)
                        out = (sc.get("output", "") or "").replace("\r", " ").replace("\n", " | ")
                        rows.append([addr, portid, state, guessed, confirmed_simple, service_group, service_purpose, service_note, probed, sid, out])
                key = (addr, portid)
                prev = summary.get(key)
                if prev is None:
                    ident_state = self._service_identification_state(guessed, confirmed_simple, probed)
                    summary[key] = {
                        "ip": addr,
                        "port": portid,
                        "state": state,
                        "guessed": guessed,
                        "simple": confirmed_simple,
                        "group": service_group,
                        "purpose": service_purpose,
                        "note": service_note,
                        "probed": probed,
                        "ident_state": ident_state,
                        "scripts": set(script_names),
                    }
                else:
        ...
        with open(csv_path, "w", encoding="utf-8-sig", newline="") as f:
            ...
            for r in rows:
                w.writerow(r)
        summary_path = (self.output_prefix or "") + "_summary.csv"
        with open(summary_path, "w", encoding="utf-8-sig", newline="") as f:
            w = csv.writer(f)
            w.writerow([
                "IP", "PORT", "포트상태", "서비스", "식별", "분류", "용도", "비고",
                "대상그룹", "조치상태", "NSE개수", "NSE목록"
            ])
            for k in sorted(summary.keys(), key=lambda x: (x[0], int(x[1]) if str(x[1]).isdigit() else 0)):
                r = summary[k]
                scripts_sorted = sorted(r["scripts"])
                w.writerow([
                    r["ip"], r["port"], r["state"], (r["simple"] or r["guessed"]), r["ident_state"], r["group"], r["purpose"], (r["probed"] or r["note"]),
                    "", "미조치", len(scripts_sorted), ",".join(scripts_sorted)
                ])
        return [csv_path, summary_path]

    def _service_identification_state(self, guessed, confirmed_simple, probed):
        merged = f"{guessed} {confirmed_simple} {probed}".lower()
        if "tcpwrapped" in merged:
            return "tcpwrapped"
        if confirmed_simple:
            return "확인"
        if probed.endswith("?") or not probed:
            return "미확인"
        return "추측"

    def _service_profile_from_name(self, service_name):
        s = (service_name or "").strip().lower()
        if not s:
            return ("unknown", "분류 필요", "서비스 식별 정보 부족 — 상세 컬럼 확인")
        rules = [
            (("http", "https", "ssl/http", "http-proxy"), ("web", "웹 서비스", "웹/웹API 가능성 — 인증·노출경로 점검")),
            (("ssh", "sftp"), ("remote-access", "원격관리", "원격 접속 계정/키/접근제어 점검")),
            (("rdp", "ms-wbt-server", "vnc"), ("remote-access", "원격관리", "원격 데스크톱 노출 — 접근 제어/망분리 점검")),
            (("microsoft-ds", "netbios-ssn", "ldap", "kerberos", "kpasswd"), ("windows-ad", "관리/인증 인프라", "도메인/인증 인프라 — 내부망 제한 권장")),
            (("domain", "dns"), ("dns", "인프라 서비스", "네임서비스 — 재귀/존전송/오픈리졸버 여부 점검")),
            (("smtp", "pop3", "imap"), ("mail", "업무 서비스", "메일 서비스 — 오픈릴레이/암호화 설정 점검")),
            (("mysql", "ms-sql", "postgresql", "mongodb", "redis", "oracle"), ("database", "데이터 저장소", "DB 접근제어/외부노출/기본계정 점검")),
            (("snmp",), ("network-mgmt", "관리용도", "네트워크 관리 프로토콜 — community 문자열 점검")),
            (("sip", "rtsp"), ("voice-video", "업무 서비스", "음성/영상 서비스 — 외부 노출 정책 점검")),
            (("ftp", "ftps", "tftp"), ("file-transfer", "업무/관리 파일전송", "익명 접근/평문 전송 여부 점검")),
        ]
        for prefixes, profile in rules:
            for p in prefixes:
                if s.startswith(p):
                    return profile
        return ("other", "기타", "서비스 성격 수동 확인 필요")
