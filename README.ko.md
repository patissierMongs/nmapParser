...
스캔이 끝나면 CSV 가 9컬럼으로 나옵니다:

`IP, PORT, 포트상태, 추측서비스, 확인서비스(short), 분류, 상세(제품/버전), NSE스크립트명, 스크립트출력`

| 컬럼 | 출처 | 의미 |
|---|---|---|
| **추측서비스** | `nmap-services` 파일 룩업 (포트번호 기준) | 정적 포트→이름 매핑 (예: `ssh`, `http`, `microsoft-ds`). 항상 채움. |
| **확인서비스(short)** | XML `<service>@name` 만 | nmap 이 실제 식별한 서비스명만 — `ssh`, `http`, `msrpc`. probe 실패 시 `?` suffix (`microsoft-ds?`). 미식별 시 빈 칸. |
| **분류** | `categories.xlsx` 룩업 | 한국어 분류 — `웹` / `원격접속` / `DBMS` / `파일공유` / `메일` / `RPC` 등. probe 실패 행은 추측서비스 이름으로 fallback 룩업. 둘 다 못 찾으면 `미분류`. |
| **상세(제품/버전)** | XML `<service>` 의 `@product` + `@version` + `@extrainfo` + `@ostype` 합침 | 깊이 보고 싶을 때만 펼치는 detail (예: `OpenSSH 9.6p1 Ubuntu 3ubuntu13.16 Ubuntu Linux`). |
| NSE스크립트명 | XML `<script>@id` | 매칭된 NSE 스크립트 ID. 여러 매칭이면 같은 포트가 여러 행으로 분리. |
| 스크립트출력 | XML `<script>@output` | NSE 스크립트 raw 출력 (개행은 ` \| ` 로 치환). |

**확인서비스(short)** 와 **상세(제품/버전)** 분리가 이번 버전의 핵심: short 컬럼이 Excel 필터/정렬의 키이고, detail 컬럼은 product/version 같은 verbose 정보 분리. **분류** 컬럼이 있어 Excel 에서 "웹만 보기" / "DBMS만 보기" 한 번에 가능.

### 분류 컬럼이 왜 필요한가
스캔 결과가 많아지면 다음 단계가 항상 "이게 웹이지, DB지, 그냥 RPC 잡음이지" 머릿속에서 분류하는 작업. `categories.xlsx` 가 이걸 미리 해줍니다:
...
기본 95개 항목 자동 동봉. Excel 로 `categories.xlsx` 편집 후 `분류 다시 불러오기` 클릭하면 즉시 반영.

### "추측 vs 확정" 의도는 그대로 유지
`추측서비스` 와 `확인서비스(short)` 두 컬럼 비교로:
- 22000번 포트 추측서비스가 `snapenetio` 인데 확인서비스가 비어 있거나 다르다면 "추측이 부정확했구나" 즉시 인지.
- 확인서비스가 `?` 로 끝나면 "probe 시도했지만 식별 실패, 추측이 전부야" 즉시 인지. `microsoft-ds?` 같은 경우.
...
| 445 | microsoft-ds | `microsoft-ds?` | 파일공유 | (빈 칸 — probe 실패) |
| 902 | iss-realsecure | `vmware-auth` | 관리 | `VMware Authentication Daemon 1.10 Uses VNC, SOAP` |
| 3389 | ms-wbt-server | `ms-wbt-server?` | 원격접속 | (빈 칸 — probe 실패) |
| 5040 | unknown | (빈 칸) | 미분류 | (빈 칸) |
