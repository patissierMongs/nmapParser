...
스캔이 끝나면 CSV 가 10컬럼 — **객관적 관찰 사실만, 판단은 외부**:
...
| **확인서비스(short)** | XML `<service>@name` 만 | nmap 이 실제 식별한 서비스명만 — `ssh`, `http`, `msrpc`. probe 실패 시 `?` suffix. 미식별 시 빈 칸. |
| **분류** | `categories.xlsx` 룩업 | 한국어 분류 — `웹` / `원격접속` / `DBMS` / `파일공유` / `메일` / `RPC` 등. probe 실패 시 추측서비스로 fallback. 둘 다 못 찾으면 `미분류`. |
| **용도** | `categories.xlsx` 룩업 | 관찰적 역할 — `관리` / `사용자` / `시스템` / `모니터링` / `내부통신` / `개발` / `보안` / `인프라`. |
| **상세(제품/버전)** | XML `<service>` 의 `@product` + `@version` + `@extrainfo` + `@ostype` 합침 | verbose 정보 — short 와 분리 (예: `OpenSSH 9.6p1 Ubuntu 3ubuntu13.16 Ubuntu Linux`). |
| NSE스크립트명 | XML `<script>@id` | 매칭된 NSE 스크립트 ID. 여러 매칭이면 같은 포트가 여러 행. |
| 스크립트출력 | XML `<script>@output` | NSE raw 출력 (개행은 ` \| ` 로 치환). |
...
| 445 | microsoft-ds | `microsoft-ds?` | 파일공유 | 시스템 | (빈 칸 — probe 실패) |
| 902 | iss-realsecure | `vmware-auth` | 관리 | 관리 | `VMware Authentication Daemon 1.10 Uses VNC, SOAP` |
| 3389 | ms-wbt-server | `ms-wbt-server?` | 원격접속 | 관리 | (빈 칸 — probe 실패) |
| 5040 | unknown | (빈 칸) | 미분류 | (빈 칸) | (빈 칸) |
