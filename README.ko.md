...
| **CSV 변환** | 스캔 후 XML → CSV 자동 변환 (옵션). 두 서비스 컬럼 (아래 참조) |
...
스캔이 끝나면 CSV 가 다음 컬럼으로 나옵니다:

`IP, PORT, 포트상태, 추측서비스(table), 확인서비스(probed), NSE스크립트명, 스크립트출력`

이 두 서비스 컬럼이 nmapParser 의 핵심 의도입니다:

| 컬럼 | 출처 | 의미 |
|---|---|---|
| **추측서비스(table)** | `nmap-services` 파일을 포트번호로 룩업 | nmap 이 *추측*하는 서비스명. "관습적으로 22번 포트 = ssh" 같은 정적 매핑. 항상 채움. |
| **확인서비스(probed)** | XML `<service>` 요소의 `method` 속성 | nmap 이 *실제로 probe* 한 결과. 세 가지 케이스: |
| | `method="probed"` | `name + product + version` (예: `ssh OpenSSH 9.6p1 Ubuntu 3ubuntu13.16`) — 확정 |
| | `method="table"` | `name?` (예: `microsoft-ds?`) — probe 시도했지만 실패. `?` 가 "이건 추측, 미확인" 신호 |
| | `<service>` 요소 없음 | 빈 칸 — 아무것도 식별 못 함 |

이게 사용자가 원했던 핵심:
- 22000번 포트의 추측서비스가 `snapenetio` 인데 실제 probe 결과는 `ssh OpenSSH ...` 라면 두 컬럼이 크게 다르고, 사용자는 "추측만 보고 판단하면 안 되겠구나" 즉시 인지.
- 445번 포트가 `microsoft-ds?` 처럼 `?` 로 끝나면 "probe 했는데 실패한 거구나, 미확정이구나" 즉시 알 수 있음 — 일반 nmap 출력에는 잘 안 보이는 정보.
...
| 445 | microsoft-ds | `microsoft-ds?` ← probe 실패, 추측만 |
| 902 | iss-realsecure | `vmware-auth VMware Authentication Daemon 1.10` ← 추측이 완전히 다름 |
| 3389 | ms-wbt-server | `ms-wbt-server?` ← probe 실패 |
| 5040 | unknown | (빈 칸) ← probe 응답 자체 없음 |
